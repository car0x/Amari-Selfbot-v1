...
def startprint():
    ...
    print(f'''{Fore.RESET}
                        ...
                        ██║░░██║██║░╚═╝░██║██║░░██║██║░░██║██║
                        ╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═╝
                       {Fore.CYAN}e v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Amari.user.name}#{Amari.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Amari.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(Amari.users)}
                       {Fore.CYAN}Guilds: {Fore.GREEN}{len(Amari.guilds)}
                       {Fore.CYAN}Prefix: {Fore.GREEN}{Amari.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    token = config.get('token')
    try:
        Amari.run(token, bot=False, reconnect=True)
        os.system(f'title (Amari Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Amari = discord.Client()
Amari = commands.Bot(description='Amari Selfbot', command_prefix=prefix, self_bot=True)

Amari.antiraid = False
Amari.msgsniper = True
Amari.slotbot_sniper = True
Amari.giveaway_sniper = True
Amari.mee6 = False
Amari.mee6_channel = None
Amari.yui_kiss_user = None
Amari.yui_kiss_channel = None
Amari.yui_hug_user = None
Amari.yui_hug_channel = None
Amari.snipe_history_dict = {}
Amari.sniped_message_dict = {}
Amari.sniped_edited_message_dict = {}
Amari.whitelisted_users = {}
Amari.copycat = None
Amari.wave = token
Amari.remove_command('help')
...
async def on_message_edit(before, after):
    await Amari.process_commands(after)


@Amari.event
async def on_message(message):
    if Amari.copycat is not None and Amari.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{Amari.user.id}>' in message.content:
        if Amari.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
    ...
    await Amari.process_commands(message)
...
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if Amari.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Amari.whitelisted_users.keys() and i.user.id in Amari.whitelisted_users[
                    guild.id].keys() and i.user.id is not Amari.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="Amari AntiNuke")
...
async def on_member_join(member):
    if Amari.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Amari.whitelisted_users.keys() and i.user.id in Amari.whitelisted_users[member.guild.id].keys():
                    return
                else:
                    await guild.ban(member, reason="Amari AntiNuke")
                    await guild.ban(i.user, reason="Amari AntiNuke")
        except Exception as e:
            print(e)
...
@Amari.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Amari.voice_clients, guild=ctx.guild)
...
@Amari.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in Amari.whitelisted_users:
            for key2 in Amari.whitelisted_users[key]:
                user = Amari.get_user(key2)
                whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + Amari.get_guild(
        ...
        for key in Amari.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in Amari.whitelisted_users[ctx.guild.id]:
                    user = Amari.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in Amari.whitelisted_users.keys():
            ...
            return
        if user.id in Amari.whitelisted_users[ctx.guild.id]:
            Amari.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Amari.get_user(user.id)
            await ctx.send(
                'Successfully unwhitelisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@Amari.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    Amari.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Amari.yui_kiss_user = user.id
        Amari.yui_kiss_channel = ctx.channel.id
        if Amari.yui_kiss_user is None or Amari.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Amari.yui_kiss_user is not None and Amari.yui_kiss_channel is not None:
            await Amari.get_channel(Amari.yui_kiss_channel).send('yui kiss ' + str(Amari.yui_kiss_user),
                                                                   delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Amari.yui_hug_user = user.id
        Amari.yui_hug_channel = ctx.channel.id
        if Amari.yui_hug_user is None or Amari.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Amari.yui_hug_user is not None and Amari.yui_hug_channel is not None:
            await Amari.get_channel(Amari.yui_hug_channel).send('yui hug ' + str(Amari.yui_hug_user),
                                                                  delete_after=0.1)
            await asyncio.sleep(60)
...
@Amari.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while Amari.mee6 is True:
        ...
        await Amari.get_channel(Amari.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@Amari.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Amari.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Amari.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Amari.slotbot_sniper = False
...
@Amari.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Amari.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Amari.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Amari.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Amari.user.id:
        return
    if Amari.msgsniper:
        ...
        if isinstance(message.channel, discord.DMChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Amari.sniped_message_dict) > 1000:
        Amari.sniped_message_dict.clear()
    if len(Amari.snipe_history_dict) > 1000:
        Amari.snipe_history_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        ...
        Amari.sniped_message_dict.update({channel_id: message_content})
        if channel_id in Amari.snipe_history_dict:
            pre = Amari.snipe_history_dict[channel_id]
            post = str(message.author) + ": " + str(message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
            Amari.snipe_history_dict.update({channel_id: pre[:-3] + post + "\n```"})
        else:
            post = str(message.author) + ": " + str(message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
            Amari.snipe_history_dict.update({channel_id: "```\n" + post + "\n```"})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Amari.event
async def on_message_edit(before, after):
    if before.author.id == Amari.user.id:
        return
    if Amari.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Amari.sniped_edited_message_dict) > 1000:
        Amari.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Amari.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Amari.command(aliases=["clearhistory"])
async def clearsnipehistory(ctx):
    await ctx.message.delete()
    del Amari.snipe_history_dict[ctx.channel.id]
    await ctx.send("Cleared Snipe History of " + ctx.channel.name, delete_after=3)

@Amari.command(aliases=["history"])
async def snipehistory(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Amari.snipe_history_dict:
        try:
            await ctx.send(Amari.snipe_history_dict[currentChannel])
        except:
            del Amari.snipe_history_dict[currentChannel]
    else:
...
async def snipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Amari.sniped_message_dict:
        await ctx.send(Amari.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!", delete_after=3)


@Amari.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Amari.sniped_edited_message_dict:
        await ctx.send(Amari.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!", delete_after=3)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="A͓̽m͓̽a͓̽r͓̽i͓̽ ͓̽S͓̽e͓̽l͓̽f͓̽B͓̽o͓̽t͓̽ ͓̽|͓̽ ͓̽P͓̽r͓̽e͓̽f͓̽i͓̽x͓̽:͓̽" + str(Amari.command_prefix),
                         icon_url=Amari.user.avatar_url)
        embed.set_thumbnail(url=Amari.user.avatar_url)
        embed.set_image(url="https://cdn.discordapp.com/attachments/926026422542270504/926396091577024523/hyped-pumped.gif")
        ...
        embed.description = f"\uD83D\uDCB0 `ANTINUKE COMMANDS`\n`> antiraid <on/off>` - toggles antinuke ({Amari.antiraid})\n`> whitelist <user>` - whitelists the specified user\n**NOTE** Whitelisting a user will completely exclude them from antinuke detections, be weary on who you whitelist.\n`> whitelisted <-g>` - see who's whitleisted and in what guild\n`> unwhitelist <user>` - unwhitelists the user\n`> clearwhitelist` - clears the whitelist hash"
        await ctx.send(embed=embed)
...
@Amari.command(aliases=["giphy", "tenor", "searchgif"])
...
@Amari.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
...
@Amari.command(aliases=["addemoji", "stealemote", "addemote"])
...
@Amari.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Amari.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Amari.copycat))
    Amari.copycat = None


@Amari.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Amari.copycat = user
    await ctx.send("Now copying " + str(Amari.copycat))


@Amari.command(aliases=["9/11", "911", "terrorist"])
...
@Amari.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@Amari.command(aliases=["spamchangegcname", "changegcname"])
...
@Amari.command(aliases=["fakename"])
...
@Amari.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Amari.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Amari.guilds:
        await guild.ack()
...
@Amari.command(aliases=["pornhublogo", "phlogo"])
...
@Amari.command(aliases=["pornhubcomment", 'phc'])
...
@Amari.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Amari.command(aliases=['pfp', 'avatar'])
...
@Amari.command(aliases=["del", "quickdel"])
...
@Amari.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Amari.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Amari.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Amari.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Amari.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Amari.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Amari.command(aliases=['dong', 'penis'])
...
@Amari.command(aliases=['changehypesquad'])
...
@Amari.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@Amari.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `Amari Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')


@Amari.command(aliases=['tokinfo', 'tdox'])
...
@Amari.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Amari.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Amari.guilds:
...
@Amari.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="amari nuked u")
    for _i in range(250):
        await ctx.guild.create_role(name="amari nuked u", color=RandomColor())


@Amari.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="amari nuked u")
        except:
            pass
...
@Amari.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="amari nuked u")
        except:
            pass


@Amari.command(aliases=["spamroles", "massroles", "addroles"])
async def massrole(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="amari nuked u", color=RandomColor(), permissions=Permissions.all())
        except:
            try:
                await ctx.guild.create_role(name="amari nuked u", color=RandomColor())
            except:
...
@Amari.command(aliases=["givemeadmin", "giveadminrole", "giveadminroles"])
...
@Amari.command(aliases=["masschannels", "masschannel", "ctc"])
...
@Amari.command(aliases=["delchannel"])
...
@Amari.command(aliases=["deleteroles"])
...
@Amari.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Amari.get_user(user.id)
    if ctx.author.id == Amari.user.id:
        return
    else:
        try:
...
@Amari.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@Amari.command(aliases=['rainbowrole'])
...
@Amari.command(aliases=["guildinfo"])
...
@Amari.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@Amari.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Amari.command(aliases=['serverbanner'])
...
@Amari.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Amari.command_prefix = str(prefix)
...
@Amari.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Amari.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Amari.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Amari.user.relationships)
    for relationship in Amari.user.relationships:
...
@Amari.command(aliases=["changeregions", "changeregion", "regionschange"])
...
async def anal(ctx):
    await ctx.message.delete()
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926405319800791040/anal-gif-at-analgifsdaily-3-gifs.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
...
async def feet(ctx):
    await ctx.message.delete()
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926405644188258314/Free-footjob-porn-gifs.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
async def boobs(ctx):
    await ctx.message.delete()
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926406004499955732/193ccb9f-bd00-4487-b1b8-b59976be0252.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
async def tits(ctx):
    await ctx.message.delete() 
    await ctx.send('https://cdn.discordapp.com/attachments/926023795486097413/926402316943376444/unknown.png')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
async def blowjob(ctx):
    await ctx.message.delete()
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926406291969163264/img_2360.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
async def lesbian(ctx):
    await ctx.message.delete() 
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926406708828450836/beauty-lezzies-playing.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
async def cumslut(ctx):
    await ctx.message.delete()
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926406948407091230/unnamed.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
@Amari.command(aliases=["vagina"])
async def pussy(ctx):
    await ctx.message.delete()
    await ctx.send('https://cdn.discordapp.com/attachments/926403657946255403/926407083715354644/unnamed_1.gif')
    res = r.json()
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
...
async def purge(ctx, amount: int = None):
    await ctx.message.delete()
    if amount is None:
        async for message in ctx.message.channel.history(limit=999).filter(lambda m: m.author == Amari.user).map(
                lambda m: m):
            try:
                await message.delete()
            except:
        ...
        async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Amari.user).map(
                lambda m: m):
            try:
                await message.delete()
            except:
...
async def stream(ctx, *, message):
    ...
    await Amari.change_presence(activity=stream)


@Amari.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await Amari.change_presence(activity=game)


@Amari.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await Amari.change_presence(
...
@Amari.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Amari.change_presence(activity=None, status=discord.Status.dnd)
...
@Amari.command(aliases=["fliptable"])
...
@Amari.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Amari.logout()


@Amari.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
